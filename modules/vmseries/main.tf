...
resource "azurerm_public_ip" "nic0" {
  ...
  name                = "pip-${var.name}${count.index + 1}-nic0"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_public_ip" "nic1" {
  ...
  name                = "pip-${var.name}${count.index + 1}-nic1"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = var.public_ip_address_allocation
  sku                 = var.sku
...
resource "azurerm_public_ip" "nic2" {
  ...
  name                = "pip-${var.name}${count.index + 1}-nic2"
  location            = var.location
  resource_group_name = var.resource_group_name
  allocation_method   = var.public_ip_address_allocation
  sku                 = var.sku
...
resource "azurerm_network_interface" "nic0" {
  count                     = var.vm_count
  name                      = "nic0-${var.name}${count.index + 1}"
  location                  = var.location
  resource_group_name       = var.resource_group_name
...
resource "azurerm_network_interface" "nic1" {
  count                     = var.vm_count
  name                      = "nic1-${var.name}${count.index + 1}"
  location                  = var.location
  resource_group_name       = var.resource_group_name
...
resource "azurerm_network_interface" "nic2" {
  count                     = var.vm_count
  name                      = "nic2-${var.name}${count.index + 1}"
  location                  = var.location
  resource_group_name       = var.resource_group_name
  enable_ip_forwarding      = true
...
resource "azurerm_network_interface" "nic3" {
  count                     = var.vm_count
  name                      = "nic3-${var.name}${count.index + 1}"
  location                  = var.location
  resource_group_name       = var.resource_group_name
  enable_ip_forwarding      = true
...
resource "azurerm_network_interface_security_group_association" "nic3" {
  count                     = var.vm_count
  network_interface_id      = element(azurerm_network_interface.nic3.*.id, count.index)
  network_security_group_id = azurerm_network_security_group.data.id
}
